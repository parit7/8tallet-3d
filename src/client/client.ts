...
const cameraViewpointNames = cameraViewpoints.map((viewpoint) => viewpoint.name);
let activeViewpointIndex = 0;
let cameraTransitionHandle: number | null = null;
let guidedTourTimer: number | null = null;

function stopGuidedTour() {
    if (guidedTourTimer !== null) {
        window.clearInterval(guidedTourTimer);
        guidedTourTimer = null;
        if (selection) {
            selection.innerText = 'Guided tour stopped';
        }
    }
}
...
function startGuidedTour() {
    stopGuidedTour();
    guidedTourTimer = window.setInterval(() => {
        goToViewpoint(activeViewpointIndex + 1);
    }, 5000);

    if (selection) {
        selection.innerText = 'Guided tour started';
    }
}
...
function goToViewpoint(index: number) {
    ...
    params.viewpointPreset = viewpoint.name;
...
const viewpointControls = {
    ...
    startGuidedTour: function() { startGuidedTour(); },
    stopGuidedTour: function() { stopGuidedTour(); },
}
...
folderViewpoints.add(params, 'viewpointPreset', cameraViewpointNames).name('Preset').onChange((value: string) => {
    const viewpointIndex = cameraViewpointNames.indexOf(value);
    if (viewpointIndex !== -1) {
        goToViewpoint(viewpointIndex);
    }
});
...
folderViewpoints.add(viewpointControls, 'startGuidedTour').name('Start guided tour');
folderViewpoints.add(viewpointControls, 'stopGuidedTour').name('Stop guided tour');
...
document.querySelector('canvas')?.addEventListener('wheel', stopGuidedTour, { passive: true });
...
function filterObj(filtertype: string, filtervalue: string, value: boolean) {
    ...
    (params as Record<string, boolean | string>)[indexEnt] = value;
